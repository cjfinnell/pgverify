...
func TestBuildFullHashQuery(t *testing.T) {
	for _, tc := range []struct {
		...
		},
		{
            ...
            FROM
                (SELECT '' AS grouper, MD5(CONCAT((extract(epoch from date_trunc('milliseconds', when))::DECIMAL * 1000000)::BIGINT::TEXT, content::TEXT, id::TEXT)) AS hash
                FROM "testSchema"."testTable" ORDER BY id, content) AS eachrow GROUP BY grouper`),
		},
	} {
		t.Run(tc.name, func(t *testing.T) {
			require.Equal(t, tc.expectedQuery, buildFullHashQuery(tc.config, tc.schemaName, tc.tableName, tc.primaryColumnNamesString, tc.columns))
		})
	}
}
